...
public class Notification {
	...
	public static InputStream midStream_current;
	...
	public static Buddy fileBuddy = null;
	...
	public static void newMessage(final Buddy from, final String msg, boolean isCurrent) {
		...
		if (isCurrent) {
			...
			if (SavedData.ringForCurrent) {
				...
				try {
					midStream_current.reset();
					Manager.createPlayer(midStream_current, "audio/mp3").start();
			...
			if (SavedData.ringForOther) {
				try {
					midStream.reset();
					Manager.createPlayer(midStream, "audio/mp3").start();
				} catch (MediaException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (IOException e) {
